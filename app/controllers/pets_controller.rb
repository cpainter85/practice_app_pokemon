class PetsController < ApplicationController
  ...
  def create
    ...
    if @pet.save
      redirect_to trainer_path(@trainer), notice: "Added a #{@pet.pokemon.name} named #{@pet.name} to #{@trainer.name}'s team"
    else
      render :new
    end
  end
  ...
  private
  def pet_params
    params.require(:pet).permit(:pokemon_id, :name, :experience_level)
  end
end
